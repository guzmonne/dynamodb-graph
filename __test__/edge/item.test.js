...
var itemFactory = require('../../src/edge/item.js');
...
describe('itemFactory()', () => {
  ...
  var data = 'Something';
  var config = { table, documentClient, tenant, maxGSIK, target };
  ...
  describe('#item', () => {
    ...
    test('should throw an error if data is not a string or a number', () => {
      expect(() => item({ node, type, data: true, target })).toThrow(
        'Data type must be a string or a number'
      );
    });

    test('should return an object', () => {
      expect(typeof item({ node, data, type, target })).toBe('object');
    });
    ...
    test('should apply the tenant value to the Node, GSIK, and TGSIK', () => {
      var options = { node, data, type, target };
